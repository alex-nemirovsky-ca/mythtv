...
void OpenGLVideo::UpdateInputFrame(const VideoFrame *frame, bool soft_bob)
{
    ...
    if (MYTHTV_YV12 == videoTextureType)
    {
        if (gl_features & kGLExtPBufObj ||
            video_dim.width() != frame->pitches[0] )
        {
            int pitches[3], offsets[3];

            pitches[0] = video_dim.width();
            pitches[2] = pitches[1] = pitches[0] / 2;

            offsets[0] = 0;
            offsets[1] = pitches[0] * video_dim.height();
            offsets[2] = offsets[1] + (pitches[1] * video_dim.height()) / 2;

            VideoFrame vf;
            init(&vf, FMT_YV12, (unsigned char *)buf,
                video_dim.width(), video_dim.height(), frame->size,
                pitches, offsets);
            copy(&vf, frame);
        }
        else
